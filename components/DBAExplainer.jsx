...
const DBA_TYPES = [
    ...
    cue: ({ active }) => (
      <svg viewBox="0 0 220 140">
        ...
        <g transform="translate(110,70)">
          <circle r={active ? 38 : 30} fill="var(--cx-coral)" style={{ transition: 'r .4s' }} />
  ...
  },
  {
    ...
    cue: ({ active }) => (
      <svg viewBox="0 0 220 140">
        <rect width="220" height="140" rx="20" fill={active ? 'var(--cx-purple-500)' : 'var(--cx-paper-soft)'} style={{ transition: 'fill .4s' }} />
          <circle cx="46" cy="70" r="18" fill="var(--cx-purple-500)" />
    ...
    cue: ({ active }) => (
      <svg viewBox="0 0 220 140">
        ...
        <g transform={`translate(110,76)`}>
          <ellipse cx="0" cy="0" rx="34" ry="38" fill="var(--cx-green-600)" />
          <circle cx="-10" cy={active ? -6 : -8} r="4" fill="var(--cx-paper)" style={{ transition: 'cy .4s' }} />
          <circle cx="10" cy={active ? -6 : -8} r="4" fill="var(--cx-paper)" style={{ transition: 'cy .4s' }} />
          <circle cx="-10" cy={active ? -5 : -7} r="1.6" fill="var(--cx-ocean)" style={{ transition: 'cy .4s' }} />
          <circle cx="10" cy={active ? -5 : -7} r="1.6" fill="var(--cx-ocean)" style={{ transition: 'cy .4s' }} />
          <path d={active ? "M-8,8 Q0,16 8,8" : "M-8,10 Q0,12 8,10"} stroke="var(--cx-ocean)" strokeWidth="2" strokeLinecap="round" fill="none" />
        </g>
      </svg>
    ),
  },
  ...
  return (
    <section className="sec" id="dbas">
      <div className="wrap-wide">
        ...
        <h2 className="sec__title">What's a <em>distinctive brand asset?</em></h2>
